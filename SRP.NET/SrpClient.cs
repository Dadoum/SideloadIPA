...
namespace SecureRemotePassword
{
    ...
    public class SrpClient
    {
        ...
        public byte[] DerivePrivateKey(byte[] salt, int iterations, string password)
        {
			...
			var x = H(s, H($":{p}"));*/
            var pbkdf2 = new Rfc2898DeriveBytes(SHA256.Create().ComputeHash(Encoding.UTF8.GetBytes(p)), s, 20763, HashAlgorithmName.SHA256);
